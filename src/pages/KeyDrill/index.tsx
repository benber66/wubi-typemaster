            ...
            {currentItem && (
              <div className="rounded-md border bg-card p-6 text-center">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  码长 {currentItem.code.length} · 弱键{' '}
                  {weakKeys.filter((k) => currentItem.code.includes(k)).join(' ')}
                </div>
                <div
                  className={`mt-3 font-mono text-lg tracking-widest transition-all ${shake ? 'animate-shake' : ''}`}
                >
              ...
              <div className="rounded-md border bg-muted/30 p-3 text-xs">
                ...
                <div className="grid grid-cols-3 gap-1 sm:grid-cols-6">
                  {keyStats.slice(0, 12).map((k) => (
                    <div
                      key={k.key}
                      className="flex items-center justify-between rounded bg-card px-2 py-1 font-mono"
                    >
                      <span className="font-semibold">{k.key}</span>
                      <span
                        className={k.errorRate > 0.3 ? 'text-destructive' : 'text-muted-foreground'}
                      >
